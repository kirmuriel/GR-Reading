...
var GRUser = function (userIdName, challengeId) {
...
GRUser.CHALLENGE = "http://www.goodreads.com/user_challenges/";
...
GRUser.prototype.getReadAndGoal = function(callback){
	var url = GRUser.CHALLENGE + this.challengeId;
	request(url, function (error, response, body) {
		if (error) return callback(error);
		var $ = cheerio.load(body);
		var stats = $($('.progressStats a')[0]).text().split(" ");
		callback(null, parseInt(stats[0]), parseInt(stats[2]));
	});
};
