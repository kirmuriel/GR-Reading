...
function getAllData(book, callback) {
	...
	var dateFinishedHeader = (book.getFinishedOn() === 0) ?  stats['avg'] +" x "+Math.round(daysToFinish)+"d = "+moment(parseInt(updatesDates[0])+daysToFinish*24*60*60*1000).format('ddd D MMM \'YY HH:mm'): moment(book.getFinishedOn()).format('ddd D MMM \'YY HH:mm');
	try {
