/* global jQuery google document myBooks myBooksData */
...
function draw(allData) {
  ...
  Object.values(allData.titles).forEach(function (title) {
  ...
  for (i = 0; i < points.length; i++) {
    var date = new Date(parseInt(points[i][0], DECIMAL_RADIX));
...
function drawData(bookHash, response) {
  ...
  for (var i = 0; i < totalDays; i++) {
    points.push([
      i + 1,
      response.data.page[i],
      Math.round(avg * (i + 1)),
      response.data.delta[i],
      Math.round(avg * 10) / 10,
      Math.round(((m * i) + b) * 10) / 10,
    ]);
  }
  ...
  var options = {
    ...
    hAxis: {
      title: 'Day', gridlines: { count: (totalDays + 1) }, minValue: 0, maxValue: (totalDays - 1),
    },
...
function setBookDataTable(bookHash, response, callback) {
  ...
  for (var i = numUpdates - 1; i >= 0; i--) {
    ...
    odd = !odd;
    styleClass = odd ? ' class="odd" ' : '';
    html += i === 0 ? ('<tr' + style + styleClass + '>') : ('<tr' + styleClass + '>');
    html += ('<td class="dateCol">' + (new Date(parseInt(date, DECIMAL_RADIX))).toUTCString().substr(0, 16) + '</td>');
